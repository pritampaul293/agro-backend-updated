...
async function run() {
  try {
    ...
    const DTCategoryCollection = database.collection(
      "Digital-technologies-category"
    );
    const ordersCollection = database.collection("All-Order");
    ...
    app.post("/addProduct", async (req, res) => {
      ...
      const result = await digitalTechnologiesCollection.insertOne({...newTin,categoryId:ObjectId(newTin.categoryId)});
      res.json(result);
    });

    // post api for adding Category by admin
    app.post("/addProductCategory", async (req, res) => {
      const newTinC = req.body;
      await DTCategoryCollection.insertOne(newTinC);
      res.json(await DTCategoryCollection.find({}).toArray());
    });

    // get api for all Category
    app.get("/productCategory", async (req, res) => {
      const result = await DTCategoryCollection.find({}).toArray();
      res.json(result);
    });
    ...
    /// E-COMMERCE ORDER FUNCTIONALITIES

    // create new order by user, 
    app.post("/newOrder", async (req,res)=>{
      const newOrderInfo = req.body;
      const result = await ordersCollection.insertOne(newOrderInfo)
      res.json(result);
    })
